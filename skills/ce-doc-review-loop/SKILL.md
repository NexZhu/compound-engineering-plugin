...
Resolve `references/loop-protocol.md` and `scripts/loop-state.mjs` relative to the directory containing this `SKILL.md`, then read and execute the protocol in order. Use the bundled helper only for fingerprinting, fresh run initialization, physical-target resolution, and compare-and-replace commit. If the protocol or helper is unavailable, return `Non-converged` with the observed reason. Invoke canonical `ce-doc-review` through one protocol-qualified route: prefer a direct callable skill mechanism; otherwise use one host-native child or isolated runner that loads and invokes the discovered canonical skill. The runner is an invocation adapter, not a substitute. Never use a generic task, copied prompt, inline persona pass, or reconstructed review. If no route qualifies, report `skill_unreachable`. Slash/dollar syntax remains output-only handoff text.
...
2. **Pass 1:** materialize the frozen raw bytes in the helper-returned disposable snapshot directory, then execute the protocol's single-route canonical invocation: exactly `ce-doc-review mode:non-interactive <snapshot-path>`. Define `reviewed_fingerprint` as SHA-256 of those frozen bytes and `result_fingerprint` as SHA-256 after canonical fixes. Require the producer-owned receipt fields, validate reviewer outcomes and fix attribution, derive the exact diff, and recheck the snapshot plus product fingerprints before any commit. Once a route may have dispatched, never retry or switch routes; a runtime, timeout, transport, or malformed-response failure is `invocation_execution_failure`.
...
- A user may explicitly accept a bounded residual only when it names the affected contract or proof, impact boundary, owner, expiry or review trigger, and fail-closed behavior. Record it durably in the Contract Matrix and run ledger, bind it to the current fingerprint, and state which specific matrix cell, graph edge, or proof obligation it satisfies. Reviewers listed by the canonical receipt in `required_reviewers` must complete; failures, timeouts, malformed returns, and missing caller-receipt fields are non-waivable coverage blockers. Optional reviewers remain report-only unless `ce-doc-review` marks them required.
