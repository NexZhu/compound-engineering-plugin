...
5. Within the remaining scope and work-unit budget, remediate all independent mechanical families allowed by the bounded invocation, one family per cycle. `cycle-authorize` is the **sole writable remediation entrypoint**: it validates the exact canonical review and family, acquires the checkout-scoped lease, checkpoints the clean bytes, and generates the **exact fixer packet**. Never dispatch a writable implementation task from findings, triage groups, or an ad-hoc prompt. **Never batch or parallelize writable defect-family fixers.** Read-only analysis may be concurrent. If an earlier invocation crashed after authorization but before `cycle-begin`, the operator may invoke `cycle-recover` with the recorded state path and lease ID; it releases only an unchanged `authorized` lease whose branch, HEAD, clean tree, and checkpoint bytes still match. Ambiguous or dirty abandoned leases stay fail-closed.
