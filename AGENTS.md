...
- **Repository identity:** Treat this checkout as the independently maintained `NexZhu/compound-engineering-plugin` repository. GitHub issues, pull requests, checks, releases, badges, and repository-targeted commands for current work must use `NexZhu/compound-engineering-plugin`, even though GitHub records `EveryInc/compound-engineering-plugin` as the network parent and an `upstream` remote may exist. Never open or update a PR against `EveryInc/compound-engineering-plugin` unless the user explicitly requests an upstream contribution. Historical links and upstream attribution may remain unchanged when they describe past work rather than the current repository target.
...
That makes cross-file isolation load-bearing rather than incidental: a test file may not depend on another file's leftovers, and any test that **writes** outside its own `mktemp` directory is a latent flake. Read-only introspection of the checkout is fine and sometimes required — `tests/bundled-script-line-endings.test.ts` runs `git ls-files -- skills` against the repo because it asserts a property of *this* checkout's tracked files, which a throwaway repo cannot have. What is banned is pointing code under test at the checkout for its *input data*: `tests/skills/ce-code-review-cross-model-routes.test.ts` used to review the live working tree, which silently coupled 31 tests to how much the contributor had staged — once a branch's diff passed the 80k-token inline budget in `cross-model-adversarial-review.sh`, the script took its large-diff skip path and those tests failed for reasons unrelated to the change. It now builds a throwaway two-commit repo with one uncommitted edit, with `GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM` neutralized so a contributor's `commit.gpgsign` or `core.hooksPath` cannot abort the fixture.
